...
This stack makes **cluster nodes** reachable on your tailnet (SSH to the node, exit-node traffic via the node WAN). It is not the Tailscale Kubernetes Operator (which proxies cluster Services).
...
Or apply with kustomize after creating the secret:
...
`k8s-install.sh` flags: `--overlay daemonset|single-node`, `--secret-env PATH`, `--image REF`.
...
Secret and ConfigMap keys: [Configuration](configuration.md).

**SSH:** OpenSSH to the **node** over the tailnet. Do not add `--ssh` to `TS_EXTRA_ARGS`.

**Exit node approval:** After the node appears online, **Machines → … → Edit route settings → Use as exit node**.
...
When you edit `scripts/ensure-exit-node-networking.sh`, `apply-ts-extra-args-local.sh`, `k8s-healthcheck.sh`, or `k8s-update.sh`, copy them into `k8s/base/scripts/` as well (kustomize cannot load files outside `k8s/base/`). `./scripts/test-k8s.sh` fails if they drift.

## How the pieces fit

```mermaid
flowchart TB
  init[initContainer exit-node-networking] --> node[node sysctl / iptables]
  ts[container tailscale hostNetwork] --> tun["/dev/net/tun"]
  ts --> hp[hostPath identity]
  wd[container watchdog] -->|every 5 min| node
  health[CronJob remote-tools-health] -->|exec ExtraArgs, rate-limited delete| ts
  upd[CronJob remote-tools-update] -->|rollout restart| ts
```
...
Env vars and CI notes: [CI and tests](ci.md).

## RBAC

The `remote-tools` ServiceAccount can list/get/delete pods, create pods/exec, and patch DaemonSets/Deployments in the `remote-tools` namespace. It cannot mutate other namespaces.

Tailscale workload pods set `automountServiceAccountToken: false` and `TS_KUBE_SECRET=""` so containerboot does **not** store node state in a Kubernetes Secret (identity stays on the node hostPath, matching Docker).
